...
interface AdminDashboardProps extends SharedData {
    ...
    recentAssignments: Array<{
        id: number;
        title?: string;
        judul?: string; // Tambahkan 'judul' untuk kompatibilitas
        deskripsi?: string; // Tambahkan deskripsi
        deadline: string;
        created_at: string;
        section?: {
            subject?: { name?: string; nama?: string }; // Support 'nama' juga
    ...
    overdueAssignments: Array<{
        ...
        section?: {
            subject?: { name?: string; nama?: string };
            guru?: { user?: { name?: string } };
        };
    }>;
    announcements: Array<{
...
interface ActivityItem {
    ...
    email?: string;
    title?: string;
    judul?: string; // Support properti judul dari backend
    deskripsi?: string; // Support properti deskripsi
    deadline?: string;
    created_at?: string;
    roles?: Array<{ name: string }>;
    section?: {
...
const fmtDateTime = (d?: string) => (d ? new Date(d).toLocaleDateString('id-ID', { hour: '2-digit', minute: '2-digit' }) : 'N/A'); // Helper baru untuk waktu
...
// [UPDATED] RecentActivityCard: Menampilkan Judul & Deskripsi Tugas
                ...
                <div className="space-y-3">
                    {safeItems.length > 0 ? (
                        safeItems.map((item, itemIndex) => {
                            // Helper variables
                            const subjectName = item.section?.subject?.nama ?? item.section?.subject?.name ?? 'Tanpa Mapel';
                            const teacherName = item.section?.guru?.user?.name ?? '—';
                            const deadlineText = fmtDateTime(item.deadline); // Format waktu lebih lengkap
                            const createdAtText = fmtDate(item.created_at);

                            // Gunakan 'judul' jika ada (standar backend), fallback ke 'title'
                            const taskTitle = item.judul ?? item.title ?? 'Tanpa Judul';
                            ...
                            return (
                                <div
                                    ...
                                    className="flex items-start justify-between rounded-lg bg-muted/50 p-3 transition hover:bg-muted/70"
                                >
                                    <div className="flex-1 space-y-1">
                                        ...
                                        {/* Tampilan untuk Assignments & Overdue (UPDATED) */}
                                        {(type === 'assignments' || type === 'overdue') && (
                                            <>
                                                <div className="flex items-center justify-between">
                                                    <p className="text-base font-medium">{taskTitle}</p>
                                                    {type === 'overdue' && (
                                                        <Badge variant="destructive" className="ml-2 shrink-0 px-1.5 py-0 text-[10px]">
                                                ...
                                                <p className="text-sm font-medium text-foreground/80">
                                                    {subjectName} <span className="text-xs font-normal text-muted-foreground">• {teacherName}</span>
                                                </p>
                        ...
                        <p className="py-6 text-center text-sm text-muted-foreground">Tidak ada data terbaru</p>
                    )}
                </div>
            </CardContent>
        </Card>
