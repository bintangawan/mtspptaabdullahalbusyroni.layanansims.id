...
import { BookOpen, Calendar, ClipboardList, Clock, FileText, MessageSquare, TrendingUp, UserCheck, Users } from 'lucide-react';
                    ...
                    schedule.map((item, index) => (
                        <div key={item.id ?? index} className="flex items-center justify-between rounded-lg bg-muted/50 p-3">
                            <div className="flex-1">
                                ...
                                {item.jadwal_today?.[0]?.ruangan && (
                                    <p className="text-sm text-muted-foreground">Ruangan: {item.jadwal_today?.[0]?.ruangan}</p>
                                )}
                            </div>
            ...
            <div className="space-y-3">
                {assignments.length > 0 ? (
                    assignments.map((assignment, index) => (
                        <Link key={assignment.id ?? index} href={`/guru/tugas/${assignment.id}`} className="block">
                            <div className="flex items-center justify-between rounded-lg bg-muted/50 p-3 transition hover:bg-muted">
                                <div className="flex-1">
                                ...
                                {safeRecentAttendance.length > 0 ? (
                                    safeRecentAttendance.map((attendance, index) => (
                                        <Link
                                            ...
                                            href={route('guru.absensi.index', attendance.section?.id)}
                                            className="block"
                                        >
                                            <div className="flex items-center justify-between rounded-lg bg-muted/50 p-3 transition hover:bg-muted">
                                                <div className="flex-1">
